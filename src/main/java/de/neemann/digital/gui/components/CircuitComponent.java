...
public class CircuitComponent extends JComponent implements ChangedListener, LibraryListener {
    ...
    private final class MouseControllerNormal extends MouseController {
        ...
        void clicked(MouseEvent e) {
            ...
            } else if (mouse.isPrimaryClick(e) && e.isShiftDown()) {
                // Shift-click: toggle selection of components and wires
                SearchResult sel = getVisualElement(pos, false);
                if (sel.getState() == SearchResult.State.FOUND) {
                    VisualElement vp = sel.getVisualElement();
                    if (highLighted.contains(vp))
                        highLighted.remove(vp);
                    else
                        highLighted.add(vp);
                } else {
                    Wire wire = getCircuit().getWireAt(pos, SIZE2);
                    if (wire != null) {
                        if (highLighted.contains(wire))
                            highLighted.remove(wire);
                        else
                            highLighted.add(wire);
                    }
                }
                boolean hasSelection = !highLighted.isEmpty();
        ...
        @Override
        public void delete() {
            if (!isLocked() && !highLighted.isEmpty()) {
                Modifications.Builder<Circuit> builder = new Modifications.Builder<>(Lang.get("menu_delete"));
                for (Drawable d : new ArrayList<>(highLighted)) {
                    if (d instanceof VisualElement)
                        builder.add(new ModifyDeleteElement((VisualElement) d));
                    else if (d instanceof Wire)
                        builder.add(new ModifyDeleteWire((Wire) d));
                }
                Modification<Circuit> mod = builder.build();
                if (mod != null) {
                    isManualScale = true;
                    modify(mod);
                }
