...
public class TableDialog extends JDialog {
    ...
    private static final Color MYGRAY_LIGHT = new Color(230, 230, 230);
    private static final Color ODDWHITE_LIGHT = new Color(255, 255, 220);
    private static final Color MYGRAY_DARK = new Color(55, 57, 65);
    private static final Color ODDWHITE_DARK = new Color(48, 50, 56);
    private static final Color WHITE_DARK = new Color(42, 44, 50);

    private static Color myGray() {
        return ThemeManager.isDarkMode() ? MYGRAY_DARK : MYGRAY_LIGHT;
    }

    private static Color oddWhite() {
        return ThemeManager.isDarkMode() ? ODDWHITE_DARK : ODDWHITE_LIGHT;
    }

    private static Color cellWhite() {
        return ThemeManager.isDarkMode() ? WHITE_DARK : Color.WHITE;
    }
    ...
    private final class CenterDefaultTableCellRenderer extends DefaultTableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            ...
            if (column < undoManager.getActual().getVars().size())
                ...
                if ((row & 4) == 0)
                    label.setBackground(cellWhite());
                else
                    label.setBackground(oddWhite());
            }

            if (ThemeManager.isDarkMode()) {
                label.setForeground(new Color(210, 212, 218));
            }
