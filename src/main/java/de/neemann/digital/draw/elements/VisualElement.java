...
public class VisualElement implements Drawable, Movable, AttributeListener {
    ...
    private void drawShape(Graphic graphic, Style highLight) {
        ...
        if (!graphic.isFlagSet(Graphic.Flag.noPinMarker)) {
            // Build a map from pin direction+index to ObservableValue using ioState
            int inIdx = 0;
            int outIdx = 0;
            for (Pin p : shape.getPins()) {
                ...
                if (ioState != null) {
                    ...
                    if (p.getDirection() == Pin.Direction.input) {
                        if (inIdx < ioState.inputCount())
                            ov = ioState.getInput(inIdx);
                        inIdx++;
                    } else {
                        if (outIdx < ioState.outputCount())
                            ov = ioState.getOutput(outIdx);
                        outIdx++;
                    }
                    if (ov != null) {
                        pinStyle = Style.getWireStyle(ov.getCopy());
                    } else {
                        pinStyle = p.getDirection() == Pin.Direction.input ? Style.WIRE : Style.WIRE_OUT;
                    }
                } else {
                    pinStyle = p.getDirection() == Pin.Direction.input ? Style.WIRE : Style.WIRE_OUT;
                }
                gr.drawCircle(p.getPos().add(-PIN, -PIN), p.getPos().add(PIN, PIN), pinStyle);
            }
        }
    }
