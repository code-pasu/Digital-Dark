...
public final class Style {
     ...
     */
    public static Style SHAPE_PIN = new Builder()
     ...
     */
    public static Style SHAPE_SPLITTER = new Builder(SHAPE_PIN).setFontSize(12).build();

    /**
     * Updates the component font size from settings.
     * Called when settings change.
     *
     * @param size the new font size
     */
    public static void updateComponentFontSize(int size) {
        SHAPE_PIN = new Builder()
                .setThickness(LINETHIN)
                .setColor(ColorKey.PINS)
                .setFontSize(size)
                .build();
        SHAPE_SPLITTER = new Builder(SHAPE_PIN).setFontSize(Math.max(size - 2, 6)).build();
    }
    /**
     * Used to draw the pin description text
     */
