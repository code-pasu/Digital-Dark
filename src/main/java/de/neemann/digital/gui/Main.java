...
import de.neemann.digital.gui.theme.ModernToolbar;
import de.neemann.digital.gui.theme.MouseShortcuts;
...
public final class Main extends JFrame implements ClosingWindowListener.ConfirmSave, FileHistory.OpenInterface, DigitalRemoteInterface, StatusInterface, ChangedListener {
    ...
    private Main(MainBuilder builder) {
        ...
        // Apply component font size from settings
        Style.updateComponentFontSize(Settings.getInstance().get(Keys.SETTINGS_COMPONENT_FONT_SIZE));
        ...
        JPanel modernStatusBar = ModernToolbar.createStatusBar(statusLabel);
        getContentPane().add(modernStatusBar, BorderLayout.SOUTH);
        ...
        // --- Component search (F2) ---
        toolBar.addSeparator();
        ComponentSearchDialog componentSearchDialog = new ComponentSearchDialog(
                this, library, circuitComponent, shapeFactory, insertHistory);
        JButton searchButton = new JButton("\uD83D\uDD0D");
        searchButton.setToolTipText(Lang.get("msg_search") + " (F2)");
        searchButton.setFocusable(false);
        searchButton.addActionListener(e -> componentSearchDialog.showDialog());
        toolBar.add(searchButton);

        // F2 global shortcut
        getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
                KeyStroke.getKeyStroke(KeyEvent.VK_F2, 0), "openComponentSearch");
        getRootPane().getActionMap().put("openComponentSearch", new AbstractAction() {
            @Override
            public void actionPerformed(ActionEvent e) {
                componentSearchDialog.showDialog();
            }
        });
        ...
        // Apply modern toolbar styling
        ModernToolbar.styleToolbar(toolBar);
        getContentPane().add(toolBar, BorderLayout.NORTH);

        // Install modern mouse shortcuts (middle-click fit, etc.)
        MouseShortcuts.install(circuitComponent);
    ...
    private JComponent createTreeComponent() {
        ...
        searchField.getTextField().getDocument().addDocumentListener(new DocumentListener() {
            ...
            public void changedUpdate(DocumentEvent documentEvent) {
                String text = searchField.getTextField().getText().trim();
                if (text.isEmpty())
                    treeModel = new LibraryTreeModel(library);
                else
    ...
    private void createEditMenu(JMenuBar menuBar) {
        ...
        ToolTipAction editSettings = new ToolTipAction(Lang.get("menu_editSettings")) {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (modified != null) {
                    ...
                    // Apply component font size
                    int compFontSize = modified.get(Keys.SETTINGS_COMPONENT_FONT_SIZE);
                    Style.updateComponentFontSize(compFontSize);
        ...
        // Keyboard shortcuts
        ToolTipAction editShortcuts = new ToolTipAction(Lang.get("menu_editShortcuts")) {
            @Override
            public void actionPerformed(ActionEvent e) {
                new ShortcutDialog(Main.this).setVisible(true);
            }
        }.setToolTip(Lang.get("menu_editShortcuts_tt"));
        edit.add(editShortcuts.createJMenuItem());
    }
    ...
    public static void main(String[] args) {
        ...
        // Initialize modern theme from saved settings
        ThemeManager.initFromSettings();
