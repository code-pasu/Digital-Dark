...
public class LayoutShape implements Shape {
    ...
    public LayoutShape(ElementTypeDescriptionCustom custom, ElementAttributes elementAttributes) throws NodeException, PinException, ElementNotFoundException {
        ...
        // In dark mode, auto-assign a vibrant color based on circuit name
        Color autoColor = ComponentColors.getColorForEmbedded(name);
        color = (autoColor != null) ? autoColor : circuit.getAttributes().get(Keys.BACKGROUND_COLOR);
    }
    ...
    public void drawTo(Graphic graphic, Style highLight) {
        ...
        // Darker border for filled components
        Color borderColor = new Color(
                Math.max((int) (color.getRed() * 0.55f), 0),
                Math.max((int) (color.getGreen() * 0.55f), 0),
                Math.max((int) (color.getBlue() * 0.55f), 0),
                color.getAlpha());
        graphic.drawPolygon(poly, Style.NORMAL.deriveColor(borderColor));

        // Contrast text style on filled background
        Color textColor = ComponentColors.getContrastText(color);
        Style textStyle = (textColor != null) ? Style.SHAPE_PIN.deriveColor(textColor) : Style.SHAPE_PIN;

        if (bottom.size() == 0)
            graphic.drawText(new Vector(width * SIZE / 2, height * SIZE + 4), name, Orientation.CENTERTOP, textStyle);
        else if (top.size() == 0)
            graphic.drawText(new Vector(width * SIZE / 2, -4), name, Orientation.CENTERBOTTOM, textStyle);
        else
            graphic.drawText(new Vector(width * SIZE / 2, height * SIZE / 2), name, Orientation.CENTERCENTER, textStyle);

        for (PinPos p : left)
            graphic.drawText(p.pos.add(4, 0), p.label, Orientation.LEFTCENTER, textStyle);
        for (PinPos p : right)
            graphic.drawText(p.pos.add(-4, 0), p.label, Orientation.RIGHTCENTER, textStyle);
        for (PinPos p : top)
            graphic.drawText(p.pos.add(0, 4), p.pos.add(0, 3), p.label, Orientation.RIGHTCENTER, textStyle);
        for (PinPos p : bottom)
            graphic.drawText(p.pos.add(0, -4), p.pos.add(0, -3), p.label, Orientation.RIGHTCENTER, textStyle);
    }
